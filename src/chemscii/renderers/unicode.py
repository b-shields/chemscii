...
class UnicodeRenderer:
    ...
    # Unicode box-drawing characters for bonds
    _HORIZONTAL = "─"
    _VERTICAL = "│"
    _DIAG_UP = "╱"
    _DIAG_DOWN = "╲"
    ...
    def __init__(self, width: int = 80, height: int = 24, padding: int = 2) -> None:
    ...
    def render(
        ...
        """Render atoms, bonds, and symbols as Unicode art.
    ...
    def _get_bond_char(self, angle: float, order: int) -> str:
        """Get the appropriate Unicode bond character for an angle.
        ...
        Returns:
            Unicode character to use for the bond.
        """
        ...
        if angle < math.pi / 8 or angle >= 7 * math.pi / 8:
            # Horizontal
            if order == 2:
                return self._DOUBLE_H
            elif order == 3:
                return self._TRIPLE_H
